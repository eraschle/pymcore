...
class HybridParameterInterface:
    ...
    def __init__(
        ...
        if enum_mapping is not None and registry is not None:
            raise ParameterConfigurationError(
                "Cannot provide both enum_mapping and registry. "
                "Use either ENUM mode or Plugin mode, not both."
            )
        
        if enum_mapping is None and registry is None:
            raise ParameterConfigurationError(
                "Must provide either enum_mapping or registry. "
                "One configuration mode is required."
            )
    ...
    def validate_value(self, role_or_key: Union[Enum, str], value: Any) -> bool:
        """
